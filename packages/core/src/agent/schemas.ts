...
const ApiKeySecurityScheme = z.strictObject({
...
const HttpSecurityScheme = z.strictObject({
...
const OAuth2FlowSchema = z.strictObject({
    authorizationUrl: z.url().optional().describe('Authorization URL for the flow'),
    tokenUrl: z.url().optional().describe('Token URL for the flow'),
    refreshUrl: z.url().optional().describe('Refresh URL for the flow'),
    scopes: z.record(z.string(), z.string()).describe('Available scopes for the OAuth2 flow'),
});

const OAuth2SecurityScheme = z.strictObject({
...
const OpenIdConnectSecurityScheme = z.strictObject({
    type: z.literal('openIdConnect').describe('Security scheme type'),
    openIdConnectUrl: z.url().describe('OpenID Connect discovery URL'),
    description: z.string().optional().describe('Description of the security scheme'),
});

const MutualTLSSecurityScheme = z.strictObject({
 ...
 */
const AgentCardSignatureSchema = z.strictObject({
 ...
 */
const DextoMetadataSchema = z.strictObject({
    authentication: z
        .strictObject({
            schemes: z
                .array(z.string())
...
export const AgentCardSchema = z.strictObject({
    ...
    url: z.url().describe('Primary endpoint URL for the agent'),
    ...
    provider: z
        .strictObject({
            ...
            url: z.url().describe('Provider organization URL'),
        })
        .optional()
        .describe('Agent provider information'),

    iconUrl: z.url().optional().describe('URL to agent icon/logo (for UI display)'),

    documentationUrl: z.url().optional().describe('URL to agent documentation'),

    additionalInterfaces: z
        .array(
            z.strictObject({
                url: z.url().describe('Endpoint URL'),
    ...
    capabilities: z
        ...
        .default({ streaming: true, stateTransitionHistory: false })
