...
const BaseContributorSchema = z.strictObject({
...
const StaticContributorSchema = z.strictObject(
...
const DynamicContributorSchema = z.strictObject(
    BaseContributorSchema.extend({
...
const FileContributorSchema = z.strictObject(
    BaseContributorSchema.extend({
        type: z.literal('file'),
        files: z
            .array(
        ...
        [StaticContributorSchema, DynamicContributorSchema, FileContributorSchema],
        {
            ...
            error: (issue) => {
                if (issue.code === 'invalid_union') {
                    return `Invalid contributor type. Expected 'static', 'dynamic', or 'file'. Note: memory contributors are now configured via the top-level 'memories' config.`;
                }
                return;
            },
        }
...
export const SessionPromptContributorSchema = z.strictObject({
...
export const SystemPromptContributorsSchema = z.strictObject({
    contributors: z
