...
- General questions`;
    ...
    describe('Contributor Type Validation', () => {
        it('should validate static contributors', () => {
            ...
            // For union schemas in Zod v4, the actual error is in errors[1] (second branch - the object branch)
            const unionError = result.error?.issues[0] as any;
            expect(unionError?.code).toBe('invalid_union');
            // Zod v4 uses 'errors' (array of arrays) instead of 'unionErrors'
            const objectErrors = unionError?.errors?.[1];
        ...
        it('should validate dynamic contributors', () => {
            ...
            expect(unionError?.code).toBe('invalid_union');
            // Zod v4 uses 'errors' (array of arrays) instead of 'unionErrors'
            // Missing required enum field reports as 'invalid_value' in Zod v4
            const objectErrors = unionError?.errors?.[1];
        ...
        it('should validate dynamic contributor source enum', () => {
            ...
            expect(unionError?.code).toBe('invalid_union');
            // Zod v4 uses 'errors' (array of arrays) instead of 'unionErrors'
            // Also, Zod v4 uses 'invalid_value' instead of 'invalid_enum_value'
            const objectErrors = unionError?.errors?.[1];
            expect(objectErrors?.[0]?.path).toEqual(['contributors', 0, 'source']);
            expect(objectErrors?.[0]?.code).toBe('invalid_value');
        });
        ...
        it('should reject invalid contributor types', () => {
            ...
            expect(unionError?.code).toBe('invalid_union');
            // Zod v4 uses 'errors' (array of arrays) instead of 'unionErrors'
            // Zod v4 uses 'invalid_union' with a 'note' property for discriminated union errors
            const objectErrors = unionError?.errors?.[1];
        ...
        it('should reject extra fields with strict validation', () => {
            ...
            expect(result.success).toBe(false);
            // In Zod v4, the outer error is 'invalid_union' because the schema is a union (string | object)
            // The actual 'unrecognized_keys' error is nested inside errors[1] (object branch)
            const unionError = result.error?.issues[0] as any;
            expect(unionError?.code).toBe('invalid_union');
            const objectErrors = unionError?.errors?.[1];
    ...
    describe('Real-world Scenarios', () => {
        ...
        it('should handle complex contributors configuration', () => {
            ...
            expect(result.contributors).toHaveLength(3);
            expect((result.contributors as any[]).map((c) => c.id)).toEqual([
                'main',
                'context',
                'date',
            ]);
